...
public class FrontControllerServlet extends HttpServlet {
	 ...
	 * 실행 후 반환된 정보를 이용해 foward 또는 redirect 방식으로 응답하게 View로 제어를 응답한다
	 * 예외 공통정책 : 예외 발생시 사용자에게는 점검중 페이지를 제공하고 시스템 콘솔에서는 예외 발생경로와 메세지를 출력한다
	 */
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			// 아래 라인 의미 => Factory Pattenr : 다양한 요청을 처리할 컨트롤러 객체를 단일한 방식으로 생성
			Controller controller = HandlerMapping.getInstance().create(command);
			// 아래 라인 의미 => 다양한 개별 컨트롤러를 단일한 방식으로 실행
			String path = controller.handleReqeust(request, response);
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 만약 웹 모듈 하위버전이라면 POST 방식의 데이터가 한글 처리가 아래와 같이 별도로 ㅣㄹ요하다.
