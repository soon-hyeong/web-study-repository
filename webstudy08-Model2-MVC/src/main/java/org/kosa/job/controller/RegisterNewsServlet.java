...
/**
 * Controller에서 view로 제어를 이동하는 방식 : redirect 방식 연
 */
...
public class RegisterNewsServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 문제점 : foward로 처리한 후 클라이언트가 새로 고침하면 뉴스가 다시 등록되는 문제가 발생하였다
//    		request.getRequestDispatcher("/news-register-result.jsp").forward(request, response);
		// 이후 redirect로 이동 후 비교해본다

		// forward는 웹 컨테이너 상에서 이동되는 반면, redirect는 응답시에 브라우저에게 지정한 url로 재요청하여 응답 결과를 보라는
		// 방식
